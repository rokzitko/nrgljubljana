...
void cmd_line(int argc, char *argv[])
{
   ...
   while ((c = getopt(argc, argv, "vm:M:r:o23nscgf:x:a:l:h:PNAB")) != -1) {
	 ...
	 ggamma = atof(optarg);
	 if (verbose) {
	    cout << "final Gaussian with gamma=" << ggamma << endl;
	 }
	 break;
	 ...
	 dgamma = atof(optarg);
	 if (verbose) {
	    cout << "final derFD with gamma=" << dgamma << endl;
	 }
	 break;
   ...
   if (verbose) {
      cout << "Nz=" << Nz << " alpha=" << alpha << " T=" << T << " omega0_ratio=" << omega0_ratio << endl;
   }
}
...
void filter()
{
   ...
   for (unsigned int j = 0; j < nr_spec; j++) {
      if (!keeppositive && vfreq[j] >= 0) {
	 vspec[j] = 0.0;
      }
      if (!keepnegative && vfreq[j] < 0) {
	 vspec[j] = 0.0;
      }
   }
}
...
void broaden(const vec & mesh, vec & a)
{
   ...
   for (int i = 0; i < nr_mesh; i++) {
      ...
      a[i] = 0.0; // clear!
      for (unsigned int j = 0; j < nr_spec; j++) {
	 a[i] += vspec[j] * bfnc(outputfreq, vfreq[j]); 
      }
   }
}
...
void gaussian_convolve(const vec & mesh, vec & a, const double sigma)
{
   ...
   if (verbose) {
      cout << "Convolving with a Gaussian. sigma=" << sigma << endl;
   }   
...
void derfd_convolve(const vec & mesh, vec & a, const double sigma)
{
   ...
   if (verbose) {
      cout << "Convolving with a derivative of the Fermi-Dirac distribution. sigma=" << sigma << endl;
   }   
...
void calc_cumulative(const vec & mesh, vec & c)
{
   ...
   if (verbose)
     cout << "Calculating cumulative spectrum." << endl;
...
int main(int argc, char *argv[])
{
   ...
   if (sumrules)
     integrals_for_sumrules();
   
   make_mesh(mesh);

   if (verbose)
     check_normalizations(mesh);

   broaden(mesh, a);
   
   if (finalgaussian)
      gaussian_convolve(mesh, a, ggamma*T);

   if (finalderfd)
      derfd_convolve(mesh, a, dgamma*T);
