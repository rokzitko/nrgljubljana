...
#define BR_L BR_L_acc
   ...
   inline double BR_G(const double e, const double ept) { 
   ...
   void gaussian_convolve(const vec &mesh, vec &a, const double sigma) {
     const int nr_mesh = mesh.size();
     if (verbose) { std::cout << "Convolving with a Gaussian. sigma=" << sigma << std::endl; }
     vec b(a); // source
     a[0]           = b[0];
   ...
   void derfd_convolve(const vec &mesh, vec &a, const double sigma) {
     ...
     if (verbose) { std::cout << "Convolving with a derivative of the Fermi-Dirac distribution. sigma=" << sigma << std::endl; }
     vec b(a); // source
     a[0]           = b[0];
     a[nr_mesh - 1] = b[nr_mesh - 1];
     for (int i = 1; i < nr_mesh - 1; i++) {
       const auto x = mesh[i];
       if (abs(x) < cutoff_ratio * sigma) {
         auto sum = 0.0, sum_kernel = 0.0;
         for (unsigned int j = 1; j < nr_mesh - 1; j++) {
           const auto y     = mesh[j];
           const auto width = (mesh[j + 1] + mesh[j]) / 2.0 - (mesh[j - 1] + mesh[j]) / 2.0;
           const auto kw    = derfd_kernel(x, y, sigma) * width;
           assert(isfinite(kw));
           sum += b[j] * kw;
           sum_kernel += kw;
         }
   ...
   void save(const std::string filename, const vec &x, const vec &y, const int SAVE_PREC = 18) {
     ...
     for (unsigned int i = 0; i < nr; i++) { F << x[i] << " " << y[i] << std::endl; }
   }

   // Estimate the weight using the trapezoidal rule. The x array must be sorted.
   auto trapez(const vec &x, const vec &y) {
     auto weight = 0.0;
     assert(x.size() == y.size());
   ...
   void check_normalizations(const vec &x) {
     ...
     for (auto z = 1e-10; z < 1.0; z *= 10) {
       ...
       const auto weight = trapez(x, y);
       std::cout << "z=" << z << " weight=" << weight << std::endl;
     }
   }
 public:
   ...
   void calc(const int COUT_PREC = 18) {
     ...
     const auto weight = trapez(mesh, a);
     std::cout << "Estimated weight (trapezoidal rule)=" << weight << std::endl;
