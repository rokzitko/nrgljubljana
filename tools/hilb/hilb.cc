...
inline double imf0(double omega, void *params) { return rho(omega) * (-y / (sqr(y) + sqr(x - omega))); }

// As imf0, but with the singularity subtracted out.
// Called from imf2
inline double imf1(double omega) { return (rho(omega) - rho(x)) * (-y / (sqr(y) + sqr(x - omega))); }

// Called from imf3p, imf3m.
// W is the relative shift in units of abs(y).
inline double imf2(double W) { return abs(y) * imf1(abs(y) * W + x); }

inline double imf3p(double rho, void *params) { return imf2(+exp(rho)) * exp(rho); }

inline double imf3m(double rho, void *params) { return imf2(-exp(rho)) * exp(rho); }

inline double ref0(double omega, void *params) { return rho(omega) * ((x - omega) / (sqr(y) + sqr(x - omega))); }

inline double ref1(double omega) { return (rho(omega) - rho(x)) * ((x - omega) / (sqr(y) + sqr(x - omega))); }

inline double ref2(double W) { return abs(y) * ref1(abs(y) * W + x); }

inline double ref3p(double rho, void *params) { return ref2(+exp(rho)) * exp(rho); }

inline double ref3m(double rho, void *params) { return ref2(-exp(rho)) * exp(rho); }

// Integrate[(-y/(y^2 + (x - omega)^2)), {omega, -B, B}]
inline double atg(double x, double y) { return atan((-B + x) / y) - atan((B + x) / y); }

// Integrate[((x - omega)/(y^2 + (x - omega)^2)), {omega, -B, B}]
inline double logs(double x, double y) { return (-log(sqr(B - x) + sqr(y)) + log(sqr(B + x) + sqr(y))) / 2.0; }
...
double calcim() {
  ...
  if (abs(y) < LIM_DIRECT) {
    ...
    if (lim1down < lim1up) {
      int status = gsl_integration_qag(&F, lim1down, lim1up, EPSABS, EPSREL, limit, GSL_INTEG_GAUSS15, w, &result1, &error1);
      handle_qag(status);
    ...
    if (veryverbose) { cout << "result1=" << result1 << " error1=" << error1 << endl; }

    double result2, error2;
    F.function = &imf3m;

    if (lim2down < lim2up) {
      int status = gsl_integration_qag(&F, lim2down, lim2up, EPSABS, EPSREL, limit, GSL_INTEG_GAUSS15, w, &result2, &error2);
    ...
    if (veryverbose) { cout << "result2=" << result2 << " error2=" << error2 << endl; }
    ...
    if (veryverbose) { cout << "result3=" << result3 << endl; }
    ...
    int status = gsl_integration_qag(&F, -B, B, EPSABS, EPSREL, limit, GSL_INTEG_GAUSS15, w, &result, &error);

    handle_qag(status);

    if (veryverbose) { cout << "Error=" << error << endl; }
...
void load_dos(char *dosfilename) {
  if (verbose) { cout << "Density of states filename: " << dosfilename << endl; }
...
void done() { gsl_integration_workspace_free(w); }
