...
class Adapt {
 public:
   ...
   // NOTE: There are two interpolation objects for the integral of rho: one for the lower boundary and one for the
   // higher boundary. This leads to a significant performance improvement.
   LinInt g;
   double x;                                          // running x
   ...
   Adapt(const Params &P, const Sign &sign) : P(P), sign(sign) { set_parameters(); }
   ...
   void load_or_calc_g() {
     Vec vecg; // XXX
     if (P.Pbool("loadg", false)) {
       vecg = load_g(g_fn(sign));
     } else {
       vecg = calc_g(vecrho);
     }
     minmaxvec(vecg, "g");
