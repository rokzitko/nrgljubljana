...
double timestep(double try_dx, double(*rhs)(double, double))
{
   ...
   if (check2) {
	 ...
	 cerr << " x=" << x 
	    ...
	    << " dEdx=" << dEdx 
	    << " dy/dx=" << dy/dx 
	    << " y*log(Lambda)=" << y * Lambda.logL() << endl;
      }
      if (dEdx > tolerance) {
	 cerr << "Aborting!" << endl;
	 exit(1);
...
double shoot_g(Vec &vecg)
{
   ...
   cout << "#  x_last=" << x << " " << "g_last/factor=" << ratio << endl;
...
// 'boundary'.
double rescale(double omega)
{
   return (1.0-boundary)*omega + boundary;
}
...
void cmd_line(int argc, char *argv[])
{
   if (argc >= 2) {
	 ...
	 break;
       default:
	 cerr << "Usage: adapt [P|N] [param_filename]" << endl;
	 exit(1);
      }
   }
   
   if (argc == 3) {
      param_fn = string(argv[2]);
   }
