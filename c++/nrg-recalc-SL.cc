...
void SymmetrySL::recalc_doublet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    ...
    Number q1 = I1.get("Q");
    Invar Ip  = Invar(q1 - 1);
    ONE23(RECALC_TAB("sl/sl-1ch-doublet.dat", SL::LENGTH_D_1CH, Invar(1)), RECALC_TAB("sl/sl-2ch-doublet.dat", SL::LENGTH_D_2CH, Invar(1)),
...
void SymmetrySL::recalc_irreduc(const DiagInfo &diag) {
  LOOP_const(diag, isp) {
    ...
    Number qp = Ip.get("Q");
    Invar I1  = Invar(qp + 1);
    ONE23(RECALC_F_TAB("sl/sl-1ch-a.dat", 0, SL::LENGTH_I_1CH), RECALC_F_TAB("sl/sl-2ch-a.dat", 0, SL::LENGTH_I_2CH);
          RECALC_F_TAB("sl/sl-2ch-b.dat", 1, SL::LENGTH_I_2CH), RECALC_F_TAB("sl/sl-3ch-a.dat", 0, SL::LENGTH_I_3CH);
          RECALC_F_TAB("sl/sl-3ch-b.dat", 1, SL::LENGTH_I_3CH); RECALC_F_TAB("sl/sl-3ch-c.dat", 2, SL::LENGTH_I_3CH));
  }
}
