...
string saverhofn(const string & prefix, int N)
{
...
void saveRho_split(size_t N, const string & prefix, const DensMatElements &rho)
{
   ...
   for (const auto &i : rho) {
     ...
     // Check each time..
     if (ONEMATRIX.bad())
       my_error("Error writing %s", fni.str().c_str());
     ONEMATRIX.close();
     cnt++;
...
void loadRho_split(size_t N, const string & prefix, DensMatElements &rho)
{
   ...
   for (size_t cnt = 0; cnt < nr; cnt++) {
	...
	my_error("Can't open file %s for reading.", fni.str().c_str());
      ...
      if (ONEMATRIX.bad())
	my_error("Error reading %s", fni.str().c_str());
      ONEMATRIX.close();
      if (P::removefiles)
	 if (remove(fni.str()))
	   my_error("Error removing %s", fni.str().c_str());
      total += diag[inv].value.size();
   }
   nrglog('H', "[total=" << total << " nr subspaces=" << nr << "]");
   if (MATRIXF.bad()) 
     my_error("Error reading %s", fn.c_str());
   MATRIXF.close();
   if (P::removefiles)
      if (remove(fn))
	my_error("Error removing %s", fn.c_str());
}
...
string unitaryfn(size_t N)
{
  return P::workdir + "/" + FN_UNITARY + tostring(N);
}
...
void store_transformations(size_t N,
			   const DiagInfo &diag)
{
...
{
   if (P::removefiles)
      remove_transformation_files_split(N);
}
          ...
          t_factor factor) // multiplicative factor that accounts for multiplicity
{
  ...
  if (!(nromega <= d1 && offset+dim <= d2)) {
     ...
     cout << "offset=" << offset << " dim=" << dim << " offset+dim=" << offset+dim
       << " d2=" << d2 << endl;
     my_error("Matrix dimensions errorin cdmI().");
  }
  const matrix_range<const Matrix> U(diagI1.matrix0, range(0, nromega), 
                              ...
                              size_t N)
{
   ...
   for (const auto & ii : dm[N-1]) {
	...
	continue;
	 ...
	 if (cnt1 && cnt2) 
	   cdmI(i, In[i], rho[In[i]], diag[In[i]], rhoPrev[I], N, coef);
      }
