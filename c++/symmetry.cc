...
{
  return (abs(ss1-ss2) <= ss3-1) &&
         (abs(ss2-ss3) <= ss1-1) &&
         (abs(ss3-ss1) <= ss2-1);
}

// The equality for U(1) symmetry.
bool u1_equality(int q1, int q2, int q3)
{
   return q1 == q2+q3;
}

bool z2_equality(int p1, int p2, int p3)
{
   return p1 == p2 * p3;
}

// C_3 quantum number: Equality modulo 3
bool c3_equality(int p1, int p2, int p3)
{
   return p1 == (p2+p3)%3;
}
...
class Symmetry 
{
 ...
 public:
   ...
   virtual void init() {
     my_error("Bug: Initializer must be defined!");
   };
   ...
   virtual bool triangle_inequality(const Invar &I1, const Invar &I2, const Invar &I3) {
     return true;
   }
   ...
   virtual bool Invar_allowed(const Invar &I) {
     return true;
   }

   virtual void makematrix(Matrix &h,
                           const Rmaxvals &qq,
                           const Invar &I,
                           const InvarVec &In) = 0;
   ...
   virtual double dynamicsusceptibility_factor(const Invar &Ip,
                                               const Invar &I1) {
     return 1.0;
   }
   ...
   virtual double dynamic_orb_susceptibility_factor(const Invar &Ip,
                                                    const Invar &I1) {
     return 1.0;
   }
   ...
   virtual double specdens_factor(const Invar &Ip,
                                  const Invar &I1) {
     return 1.0;
   }

   virtual double specdensquad_factor(const Invar &Ip,
				      const Invar &I1) {
     return 1.0;
   }
  ...
  virtual void recalc_irreduc(const DiagInfo &diag) {
    my_error("Not implemented.");
  }

  virtual void recalc_irreduc_substeps(const DiagInfo &diag, int M) {
    my_error("Not implemented.");
  }

  virtual void recalc_doublet(DiagInfo &diag,
                              MatrixElements &cold,
                              MatrixElements &cnew) {
    my_error("Not implemented.");
  }

  virtual void recalc_triplet(DiagInfo &diag,
                              MatrixElements &cold,
                              MatrixElements &cnew) {
    my_error("Not implemented.");
  }

  virtual void recalc_orb_triplet(DiagInfo &diag,
                                  MatrixElements &cold,
                                  MatrixElements &cnew) {
    my_error("Not implemented.");
  }

  virtual void recalc_quadruplet(DiagInfo &diag,
				 MatrixElements &cold,
				 MatrixElements &cnew) {
     my_error("Not implemented.");
  }

  virtual void recalc_global(DiagInfo &diag,
			     string name,
			     MatrixElements &cnew) {
     my_error("Not implemented.");
  }
...
inline size_t mult(const Invar &I) 
{
   return Sym->mult(I);
}

// Add DECL declaration in each symmetry class
#define DECL void makematrix(Matrix &h, const Rmaxvals &qq, \
                             const Invar &I, const InvarVec &In); \
             void recalc_irreduc(const DiagInfo &diag)

// Optional declaration
#define HAS_DOUBLET void recalc_doublet(DiagInfo &diag, \
                              MatrixElements &cold, MatrixElements &cnew)
#define HAS_TRIPLET void recalc_triplet(DiagInfo &diag, \
                              MatrixElements &cold, MatrixElements &cnew)
#define HAS_ORB_TRIPLET void recalc_orb_triplet(DiagInfo &diag, \
                              MatrixElements &cold, MatrixElements &cnew)
#define HAS_QUADRUPLET void recalc_quadruplet(DiagInfo &diag, \
                              MatrixElements &cold, MatrixElements &cnew)
#define HAS_GLOBAL void recalc_global(DiagInfo &diag, \
                              string name, MatrixElements &cnew)
...
// Helper functions
void check_abs_diff(const Invar &Ip, const Invar &I1, 
                    const string &what, int diff) {
  const int a = Ip.get(what);
  const int b = I1.get(what);
  my_assert( abs(b-a) == diff );
}

void check_diff(const Invar &Ip, const Invar &I1, 
                const string &what, int diff) {
  const int a = Ip.get(what);
  const int b = I1.get(what);
  my_assert(b-a == diff);
