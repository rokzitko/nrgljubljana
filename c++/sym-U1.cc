...
#include "u1/u1-3ch-QN.dat"
  ...
  void makematrix_pol2x2(Matrix &h, const Rmaxvals &qq, const Invar &I, const InvarVec &In);
...
#define OFFDIAG_DO(i, j, ch, factor0) offdiag_function(i, j, ch, 1, t_matel(factor0) * xiDOWN(STAT::N, ch), h, qq, In)

// UPDO -> <f> from previous site for spin UP (index fnr=0)
#define OFFDIAG_UPDO(i, j, ch, factor0) offdiag_function(i, j, ch, 0, t_matel(factor0) * xiUPDO(STAT::N, ch), h, qq, In)

// DOUP -> <f> from previous site for spin DO (index fnr=1)
#define OFFDIAG_DOUP(i, j, ch, factor0) offdiag_function(i, j, ch, 1, t_matel(factor0) * xiDOUP(STAT::N, ch), h, qq, In)
...
#define DIAG_DOUP(i, j, ch, factor) diag_offdiag_function(i, j, ch, t_matel(factor) * zetaDOUP(STAT::N + 1, ch), h, qq)
...
void SymmetryU1::makematrix_polarized(Matrix &h, const Rmaxvals &qq, const Invar &I, const InvarVec &In) {
  switch (channels) {
    case 1:
...
void SymmetryU1::makematrix_pol2x2(Matrix &h, const Rmaxvals &qq, const Invar &I, const InvarVec &In) {
  switch (channels) {
    case 1:
#include "u1/u1-1ch-offdiag-UP.dat"
#include "u1/u1-1ch-offdiag-DO.dat"
...
#define OFFDIAG_DO(i, j, ch, factor) offdiag_function(i, j, ch, 0, t_matel(factor) * xi(STAT::N, ch), h, qq, In)
#define OFFDIAG_UP(i, j, ch, factor) offdiag_function(i, j, ch, 1, t_matel(factor) * xi(STAT::N, ch), h, qq, In)

void SymmetryU1::makematrix_nonpolarized(Matrix &h, const Rmaxvals &qq, const Invar &I, const InvarVec &In) {
  switch (channels) {
    case 1:
#include "u1/u1-1ch-offdiag-UP.dat"
#include "u1/u1-1ch-offdiag-DO.dat"
...
void SymmetryU1::makematrix(Matrix &h, const Rmaxvals &qq, const Invar &I, const InvarVec &In) {
  if (P::pol2x2) {
