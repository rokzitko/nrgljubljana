...
class Params {
  ...
  */
  ...
  param<bool> dumpenergies{"dumpenergies", "Dump (all) energies to file [energies.nrg]?", "false", all};  // N
  ...
  size_t Nlen = 0;         // Nlen=Nmax for regular calculations. Nlen=1 for ZBW [unsupported]. Length of wn, wnfactor, ZnD and dm vectors.

  // obsolete: [[nodiscard]] bool ZBW() const noexcept { return Nmax == Ninit; } // Zero-bandwidth calculation
  ...
  bool do_recalc_kept(const RUNTYPE &runtype) const noexcept {   // kept: Recalculate using vectors kept after truncation
    return strategy == "kept" && !(cfs_or_fdm_flags() && runtype == RUNTYPE::DMNRG); 
  }
  ...
  // Energy window for the patching procedure [FT and DMNRG algorithms].
