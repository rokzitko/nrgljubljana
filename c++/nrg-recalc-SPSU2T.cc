...
void SymmetrySPSU2T::recalc_doublet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    Invar I1   = INVAR(is1);
    Sspin ss1  = I1.get("SS");
    Tangmom t1 = I1.get("T");
    ...
    Ip = Invar(ss1 + 1, t1 - 1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2t/spsu2t-doubletp-1.dat"
                << ", len=" << SPSU2T::LENGTH_D_3CH_1 << ", Iop=" << Invar(1, 2, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "spsu2t/spsu2t-doubletp-1.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == SPSU2T::LENGTH_D_3CH_1);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, SPSU2T::LENGTH_D_3CH_1, Invar(1, 2, 1));
      }
    };
...
#include "spsu2t/spsu2t-doubletm-1.dat"
    ...
    Ip = Invar(ss1 + 1, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2t/spsu2t-doubletp0.dat"
                << ", len=" << SPSU2T::LENGTH_D_3CH_0 << ", Iop=" << Invar(1, 2, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "spsu2t/spsu2t-doubletp0.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == SPSU2T::LENGTH_D_3CH_0);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, SPSU2T::LENGTH_D_3CH_0, Invar(1, 2, 1));
      }
    };
...
void SymmetrySPSU2T::recalc_irreduc(const DiagInfo &diag) {
  ...
  LOOP_const(diag, isp) {
    ...
    I1 = Invar(ssp + 1, tp + 1);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2t/spsu2t-spinup+1.dat"
                << ", ch=" << 0 << ", len=" << SPSU2T::LENGTH_I_3CH_0 << ")");
      if (diag.count(I1)) {
...
#include "spsu2t/spsu2t-spinup+1.dat"
    ...
    I1 = Invar(ssp + 1, tp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2t/spsu2t-spinup0.dat"
                << ", ch=" << 0 << ", len=" << SPSU2T::LENGTH_I_3CH_1 << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2t/spsu2t-spinup0.dat"
    ...
    I1 = Invar(ssp + 1, tp - 1);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2t/spsu2t-spinup-1.dat"
                << ", ch=" << 0 << ", len=" << SPSU2T::LENGTH_I_3CH_2 << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2t/spsu2t-spinup-1.dat"
        };
...
void SymmetrySPSU2T::recalc_triplet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    Invar I1   = INVAR(is1);
    Sspin ss1  = I1.get("SS");
    Tangmom t1 = I1.get("T");
    ...
    Ip = Invar(ss1, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2t/spsu2t-triplets.dat"
                << ", len=" << SPSU2T::LENGTH_T0_3CH << ", Iop=" << Invar(3, 0) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "spsu2t/spsu2t-triplets.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == SPSU2T::LENGTH_T0_3CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, SPSU2T::LENGTH_T0_3CH, Invar(3, 0));
      }
    };
    ...
    Ip = Invar(ss1 + 2, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2t/spsu2t-tripletp.dat"
                << ", len=" << SPSU2T::LENGTH_Tpm_3CH << ", Iop=" << Invar(3, 0) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "spsu2t/spsu2t-tripletp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == SPSU2T::LENGTH_Tpm_3CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, SPSU2T::LENGTH_Tpm_3CH, Invar(3, 0));
      }
    };
