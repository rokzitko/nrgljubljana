      ...
      case 1: {
        {
          nrglog('f',
                 "RECALC(fn="
                    << "u1/u1-1ch-doublet.dat"
                    << ", len=" << U1::LENGTH_D_1CH << ", Iop=" << Invar(1) << ")");
          if (diag.count(Ip)) {
            struct Recalc recalc_table[] = {
#include "u1/u1-1ch-doublet.dat"
            };
            BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == U1::LENGTH_D_1CH);
            recalc_general(diag, cold, cnew, I1, Ip, recalc_table, U1::LENGTH_D_1CH, Invar(1));
          }
        }
      } break;
      case 2: {
        {
          nrglog('f',
                 "RECALC(fn="
                    << "u1/u1-2ch-doublet.dat"
                    << ", len=" << U1::LENGTH_D_2CH << ", Iop=" << Invar(1) << ")");
          if (diag.count(Ip)) {
            struct Recalc recalc_table[] = {
#include "u1/u1-2ch-doublet.dat"
            };
            BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == U1::LENGTH_D_2CH);
            recalc_general(diag, cold, cnew, I1, Ip, recalc_table, U1::LENGTH_D_2CH, Invar(1));
          }
        }
      } break;
      case 3: {
        {
          nrglog('f',
                 "RECALC(fn="
                    << "u1/u1-3ch-doublet.dat"
                    << ", len=" << U1::LENGTH_D_3CH << ", Iop=" << Invar(1) << ")");
          if (diag.count(Ip)) {
            struct Recalc recalc_table[] = {
#include "u1/u1-3ch-doublet.dat"
            };
            BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == U1::LENGTH_D_3CH);
            recalc_general(diag, cold, cnew, I1, Ip, recalc_table, U1::LENGTH_D_3CH, Invar(1));
          }
        }
      } break;
