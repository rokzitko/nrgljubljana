...
void SymmetryISOLR::recalc_doublet(const DiagInfo &diag, const MatrixElements &cold, MatrixElements &cnew) {
  for(const auto &[I1, eig]: diag) {
    ...
    RECALC_TAB("isolr/isolr-2ch-doubletmp.dat", ISOLR::LENGTH_D_2CH, Invar(2, 2, +1));

    Ip = Invar(ii1 - 1, ss1 - 1, p1);
    RECALC_TAB("isolr/isolr-2ch-doubletmm.dat", ISOLR::LENGTH_D_2CH, Invar(2, 2, +1));

    Ip = Invar(ii1 + 1, ss1 + 1, p1);
    RECALC_TAB("isolr/isolr-2ch-doubletpp.dat", ISOLR::LENGTH_D_2CH, Invar(2, 2, +1));
...
void SymmetryISOLR::recalc_triplet(const DiagInfo &diag, const MatrixElements &cold, MatrixElements &cnew) {
  for(const auto &[I1, eig]: diag) {
    ...
    Ip = Invar(ii1, ss1, p1);
    RECALC_TAB("isolr/isolr-2ch-triplets.dat", ISOLR::LENGTH_T0_2CH, Invar(1, 3, +1));

    Ip = Invar(ii1, ss1 + 2, p1);
    RECALC_TAB("isolr/isolr-2ch-tripletp.dat", ISOLR::LENGTH_Tpm_2CH, Invar(1, 3, +1));
