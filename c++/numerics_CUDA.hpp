...
template<scalar S> struct CudaDirectCopyCompatible : std::false_type {};
template<> struct CudaDirectCopyCompatible<double> : std::true_type {};
template<> struct CudaDirectCopyCompatible<std::complex<double>>
    : std::bool_constant<sizeof(std::complex<double>) == sizeof(cuDoubleComplex) && alignof(std::complex<double>) == alignof(cuDoubleComplex)> {};

template<scalar S>
inline constexpr bool cuda_direct_copy_compatible_v = CudaDirectCopyCompatible<S>::value;

template<scalar S, Eigen_matrix EM>
void cuda_mult_copy_to_device(const EM &M, CudaMultBuffer<typename CudaMultTraits<S>::cuda_type> &buffer, const char *what) {
  using CT = typename CudaMultTraits<S>::cuda_type;
  const auto elements = size1(M) * size2(M);
  buffer.ensure(elements);
  if constexpr (cuda_direct_copy_compatible_v<S>) {
    cuda_mult_check(cudaMemcpy(buffer.data(), reinterpret_cast<const CT *>(M.data()), CudaMultBuffer<CT>::bytes(elements), cudaMemcpyHostToDevice), what);
  } else {
    auto host = cuda_mult_copy_matrix<S>(M);
    cuda_mult_check(cudaMemcpy(buffer.data(), host.data(), CudaMultBuffer<CT>::bytes(elements), cudaMemcpyHostToDevice), what);
  }
}

template<scalar S, Eigen_matrix EM>
void cuda_mult_copy_from_device(EM &M, const typename CudaMultTraits<S>::cuda_type *device, const char *what) {
  using CT = typename CudaMultTraits<S>::cuda_type;
  const auto elements = size1(M) * size2(M);
  if constexpr (cuda_direct_copy_compatible_v<S>) {
    cuda_mult_check(cudaMemcpy(reinterpret_cast<CT *>(M.data()), device, CudaMultBuffer<CT>::bytes(elements), cudaMemcpyDeviceToHost), what);
  } else {
    std::vector<CT> host(elements);
    cuda_mult_check(cudaMemcpy(host.data(), device, CudaMultBuffer<CT>::bytes(elements), cudaMemcpyDeviceToHost), what);
    cuda_mult_copy_back<S>(M, host);
  }
}
...
template<scalar S> class CudaRecalcScope {
 public:
  ...
  void register_matrix(const EM &M) {
    ...
    auto cached = std::make_unique<CachedMatrix>(size1(M) * size2(M));
    cached->rows = size1(M);
    cached->cols = size2(M);
    cuda_mult_copy_to_device<S>(M, cached->buffer, "cudaMemcpy H2D cached matrix");
  ...
  const CT *upload_uncached(const EM &M, CudaMultBuffer<CT> &buffer, const char *what) {
    cuda_mult_copy_to_device<S>(M, buffer, what);
    return buffer.data();
  }
...
template<scalar S, Eigen_matrix EM> class CudaRecalcAccumulator {
 public:
  ...
  explicit CudaRecalcAccumulator(EM &M) : M_(M), ctx_(CudaRecalcScope<S>::current()) {
    ...
    device_ = &ctx_->output_buffer(size1(M_) * size2(M_));
    cuda_mult_copy_to_device<S>(M_, *device_, "cudaMemcpy H2D accumulator");
  }
  ...
  void copy_back() {
    if (!active()) return;
    cuda_mult_copy_from_device<S>(M_, device_->data(), "cudaMemcpy D2H accumulator");
  }
...
void product_CUDA(EM &M, const t_coef factor, const EM &A, const EM &B) {
  ...
  CudaMultBuffer<CT> d_M(size1(M) * size2(M)), d_A(size1(A) * size2(A)), d_B(size1(B) * size2(B));
  cuda_mult_copy_to_device<S>(M, d_M, "cudaMemcpy H2D M");
  cuda_mult_copy_to_device<S>(A, d_A, "cudaMemcpy H2D A");
  cuda_mult_copy_to_device<S>(B, d_B, "cudaMemcpy H2D B");
  const auto alpha = Traits::convert(S(factor));
  const auto beta = Traits::convert(S(1.0));
  CublasHandle handle;
...
void transform_CUDA(EM &M, const t_coef factor, const EM &A, const EM &O, const EM &B) {
  ...
  CudaMultBuffer<CT> d_M(size1(M) * size2(M)), d_A(size1(A) * size2(A)), d_O(size1(O) * size2(O)), d_B(size1(B) * size2(B)), d_tmp(static_cast<size_t>(b) * static_cast<size_t>(m));
  cuda_mult_copy_to_device<S>(M, d_M, "cudaMemcpy H2D M");
  cuda_mult_copy_to_device<S>(A, d_A, "cudaMemcpy H2D A");
