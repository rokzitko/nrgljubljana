...
#include "iso2lr/iso2lr-2ch-spinup-isoupa.dat"
    ...
    recalc_f(diag, iterinfo.opch[0][0], Ip, I1, recalc_table, ISO2LR::LENGTH_I_2CH);
  }
};
    {
...
#include "iso2lr/iso2lr-2ch-spinup-isoupb.dat"
    ...
    BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISO2LR::LENGTH_I_2CH);
    recalc_f(diag, iterinfo.opch[1][0], Ip, I1, recalc_table, ISO2LR::LENGTH_I_2CH);
  }
};
...
void SymmetryISO2LR::recalc_doublet(const DiagInfo &diag, const MatrixElements &cold, MatrixElements &cnew) {
...
void SymmetryISO2LR::recalc_triplet(const DiagInfo &diag, const MatrixElements &cold, MatrixElements &cnew) {
  for(const auto &[I1, eig]: diag) {
    Ispin ii1 = I1.get("II");
    Sspin ss1 = I1.get("SS");
    int p1    = I1.get("P");
