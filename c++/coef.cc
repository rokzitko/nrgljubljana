...
struct Coef {
   ...
   t_coef xiUP(size_t N, size_t ch) const { return xi(N, ch); }
   t_coef xiDOWN(size_t N, size_t ch) const { return xi(N, ch + P.channels); }
   t_coef zetaUP(size_t N, size_t ch) const { return zeta(N, ch); }
   t_coef zetaDOWN(size_t N, size_t ch) const { return zeta(N, ch + P.channels); }
   ...
   t_coef xiUPDO(size_t N, size_t ch) const { return xi(N, ch + 2 * P.channels); }
   t_coef xiDOUP(size_t N, size_t ch) const {  return xi(N, ch + 3 * P.channels); }
   t_coef zetaUPDO(size_t N, size_t ch) const { return zeta(N, ch + 2 * P.channels); }
   t_coef zetaDOUP(size_t N, size_t ch) const { return zeta(N, ch + 3 * P.channels); }
