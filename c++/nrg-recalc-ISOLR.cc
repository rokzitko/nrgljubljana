...
void SymmetryISOLR::recalc_irreduc(const DiagInfo &diag) {
  ...
  LOOP_const(diag, isp) {
    ...
    I1 = Invar(iip + 1, ssp + 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isoupa.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
...
#include "isolr/isolr-2ch-spinup-isoupa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isoupb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isoupb.dat"
    ...
    I1 = Invar(iip + 1, ssp - 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isoupa.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isoupa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isoupb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isoupb.dat"
    ...
    I1 = Invar(iip - 1, ssp + 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isodowna.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isodowna.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isodownb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isodownb.dat"
    ...
    I1 = Invar(iip - 1, ssp - 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isodowna.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isodowna.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isodownb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isodownb.dat"
    ...
    I1 = Invar(iip + 1, ssp + 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isoupdiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isoupdiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isoupdiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isoupdiffb.dat"
    ...
    I1 = Invar(iip + 1, ssp - 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isoupdiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isoupdiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isoupdiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isoupdiffb.dat"
    ...
    I1 = Invar(iip - 1, ssp + 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isodowndiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isodowndiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spinup-isodowndiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spinup-isodowndiffb.dat"
    ...
    I1 = Invar(iip - 1, ssp - 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isodowndiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isolr/isolr-2ch-spindown-isodowndiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isolr/isolr-2ch-spindown-isodowndiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
...
void SymmetryISOLR::recalc_doublet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    Invar I1  = INVAR(is1);
    Ispin ii1 = I1.get("II");
    Sspin ss1 = I1.get("SS");
    ...
    Ip = Invar(ii1 - 1, ss1 + 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isolr/isolr-2ch-doubletmp.dat"
                << ", len=" << ISOLR::LENGTH_D_2CH << ", Iop=" << Invar(2, 2, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isolr/isolr-2ch-doubletmp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISOLR::LENGTH_D_2CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, ISOLR::LENGTH_D_2CH, Invar(2, 2, +1));
      }
    };
...
void SymmetryISOLR::recalc_triplet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    Invar I1  = INVAR(is1);
    Ispin ii1 = I1.get("II");
    Sspin ss1 = I1.get("SS");
    ...
    Ip = Invar(ii1, ss1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isolr/isolr-2ch-triplets.dat"
                << ", len=" << ISOLR::LENGTH_T0_2CH << ", Iop=" << Invar(1, 3, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isolr/isolr-2ch-triplets.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISOLR::LENGTH_T0_2CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, ISOLR::LENGTH_T0_2CH, Invar(1, 3, +1));
      }
    };
    ...
    Ip = Invar(ii1, ss1 + 2, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isolr/isolr-2ch-tripletp.dat"
                << ", len=" << ISOLR::LENGTH_Tpm_2CH << ", Iop=" << Invar(1, 3, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isolr/isolr-2ch-tripletp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISOLR::LENGTH_Tpm_2CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, ISOLR::LENGTH_Tpm_2CH, Invar(1, 3, +1));
      }
    };
