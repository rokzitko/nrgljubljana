...
public:
   DiagInfo<S> diagonalisations(const Step &step, const Opch<S> &opch, const Coef<S> &coef, const DiagInfo<S> &diagprev, const Output<S> &output,
      ...
      const auto nr = tasks_by_size.size();
      size_t itask = 0;
#if NRG_ENABLE_APP_OPENMP
...
#pragma omp parallel for schedule(dynamic) num_threads(nth)
      ...
      for (itask = 0; itask < nr; itask++) {
        const auto [dim, I] = tasks_by_size[itask];
...
#pragma omp critical
        { nrglog('(', "[OpenMP] Diagonalizing " << I << " dim=" << dim << " (task " << itask + 1 << "/" << nr << ", thread " << thid << ")"); }
#else
        nrglog('(', "[serial scheduler] Diagonalizing " << I << " dim=" << dim << " (task " << itask + 1 << "/" << nr << ")");
#endif
