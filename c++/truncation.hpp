...
void truncate_prepare(const Step &step, DiagInfo<S> &diag, MF mult, const Params &P) {
  const auto Cmax = highest_retained(step, diag, P);
  for (auto &[I, eig] : diag)
    diag[I].truncate_prepare(step.last() && P.keep_all_states_in_last_step()
                             ? eig.getnrcomputed()
                             : ranges::count_if(eig.values.all_crit(), [Cmax](const double c) { return c <= Cmax; }));
  std::cout << "Cmax=" << Cmax / step.unscale() << " ";
  truncate_stats ts(diag, mult);
  ts.report();
  if (ranges::any_of(diag, [Cmax](const auto &d) {
        const auto &[I, eig] = d;
        return eig.getnrkept() == eig.getnrcomputed() && eig.values.highest_crit() != Cmax && eig.getnrcomputed() < eig.getdim();
      }))
    throw NotEnough();
