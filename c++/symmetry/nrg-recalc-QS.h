...
MatrixElements<SC> SymmetryQS<SC>::recalc_doublet(const DiagInfo<SC> &diag, const QSrmax &qsrmax, const MatrixElements<SC> &cold) {
...
#include "qs/qs-4ch-spinupd.dat"
      ...
      opch[3][0][II] = this->recalc_f(diag, qsrmax, I1, Ip, recalc_table);
    }
  }
} } break;
  default: my_assert_not_reached();
...
template<typename SC>
MatrixElements<SC> SymmetryQS<SC>::recalc_triplet(const DiagInfo<SC> &diag, const QSrmax &qsrmax, const MatrixElements<SC> &cold) {
  MatrixElements<SC> cnew;
  if (!P.substeps) {
    for(const auto &[I1, eig]: diag) {
      Number q1 = I1.get("Q");
...
#include "qs/qs-1ch-triplets.dat"
      ...
      cnew[II] = this->recalc_general(diag, qsrmax, cold, I1, Ip, recalc_table, Invar(0, 3));
    }
  }
} } break;
  case 2: { {
...
template<typename SC>
void SymmetryQS<SC>::recalc_global(const Step &step, const DiagInfo<SC> &diag, const QSrmax &qsrmax, 
