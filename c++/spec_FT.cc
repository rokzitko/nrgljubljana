...
void SPEC_FT::calc(const Eigen &diagIp, const Eigen &diagI1, const Matrix &op1II, const Matrix &op2II, const BaseSpectrum &bs, t_factor spinfactor,
                   spCS_t cs, const Invar &Ip, const Invar &I1, const DensMatElements &) {
  ...
  for (size_t r1 = 0; r1 < diagI1.getnr(); r1++) {
    ...
    for (size_t rp = 0; rp < diagIp.getnr(); rp++) {
      ...
      DELTA d;
      d.weight = (spinfactor / Zft) * CONJ_ME(op1II(r1, rp)) * op2II(r1, rp) * ((-sign) * exp(-E1 * scT) + exp(-Ep * scT));
      d.energy = E1 - Ep;
...
void SPEC_FTmats::calc(const Eigen &diagIp, const Eigen &diagI1, const Matrix &op1II, const Matrix &op2II, const BaseSpectrum &bs,
                       t_factor spinfactor, spCS_t cs, const Invar &Ip, const Invar &I1, const DensMatElements &) {
  ...
  for (size_t r1 = 0; r1 < diagI1.getnr(); r1++) {
    ...
    for (size_t rp = 0; rp < diagIp.getnr(); rp++) {
      const t_eigen Ep = diagIp.value(rp);
      DELTA d;
      d.weight = (spinfactor / Zft) * CONJ_ME(op1II(r1, rp)) * op2II(r1, rp) * ((-sign) * exp(-E1 * scT) + exp(-Ep * scT)); // sign!
      d.energy = E1 - Ep;
#pragma omp parallel for schedule(static)
      for (size_t n = 1; n < cutoff; n++) csm->add(n, d.weight / (cmpl(0, ww(n, bs.mt)) - scale * d.energy));
  ...
  for (size_t r1 = 0; r1 < diagI1.getnr(); r1++) {
    ...
    for (size_t rp = 0; rp < diagIp.getnr(); rp++) {
      ...
      value += beta * (spinfactor / STAT::Zgt) * CONJ_ME(op1II(r1, rp)) * op2II(r1, rp)
         / (exp(+E1 * STAT::scale * beta) + exp(+Ep * STAT::scale * beta)) * pow((E1 - Ep) * STAT::scale, power);
    } // loop over r1
  }   // loop over rp
