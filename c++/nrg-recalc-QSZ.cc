...
#include "qsz/qsz-1ch-doubletp.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_D_1CH, Invar(1, -1));
  }
} } break;
  case 2: { {
...
#include "qsz/qsz-2ch-doubletp.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_D_2CH, Invar(1, -1));
  }
} } break;
  case 3: { {
...
#include "qsz/qsz-3ch-doubletp.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_D_3CH, Invar(1, -1));
  }
} } break;
...
void SymmetryQSZ::recalc_irreduc(const DiagInfo &diag, const QSrmax &qsrmax, Opch &opch) {
  my_assert(!substeps);
...
#include "qsz/qsz-1ch-spinupa.dat"
    ...
    recalc_f(diag, qsrmax, opch[0][0], Ip, I1, recalc_table, QSZ::LENGTH_I_1CH);
  }
} } break;
  case 2: { {
...
#include "qsz/qsz-2ch-spinupa.dat"
    ...
    recalc_f(diag, qsrmax, opch[0][0], Ip, I1, recalc_table, QSZ::LENGTH_I_2CH);
  }
};
      	   {
...
#include "qsz/qsz-2ch-spinupb.dat"
    ...
    BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QSZ::LENGTH_I_2CH);
    recalc_f(diag, qsrmax, opch[1][0], Ip, I1, recalc_table, QSZ::LENGTH_I_2CH);
  }
} } break;
  case 3: { {
...
#include "qsz/qsz-3ch-spinupa.dat"
    ...
    recalc_f(diag, qsrmax, opch[0][0], Ip, I1, recalc_table, QSZ::LENGTH_I_3CH);
  }
};
           {
...
#include "qsz/qsz-3ch-spinupb.dat"
    ...
    BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QSZ::LENGTH_I_3CH);
    recalc_f(diag, qsrmax, opch[1][0], Ip, I1, recalc_table, QSZ::LENGTH_I_3CH);
  }
};
      	   {
...
#include "qsz/qsz-3ch-spinupc.dat"
    ...
    BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QSZ::LENGTH_I_3CH);
    recalc_f(diag, qsrmax, opch[2][0], Ip, I1, recalc_table, QSZ::LENGTH_I_3CH);
  }
} } break;
  default: my_assert_not_reached();
  };
...
void SymmetryQSZ::recalc_irreduc_substeps(const DiagInfo &diag, const QSrmax &qsrmax, Opch &opch, int M) {
  my_assert(substeps);
  for(const auto &[Ip, eig]: diag) {
    Number qp   = Ip.get("Q");
    SZspin sszp = Ip.get("SSZ");
...
#include "qsz/qsz-1ch-spinupa.dat"
    ...
    BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QSZ::LENGTH_I_1CH);
    recalc_f(diag, qsrmax, opch[M][0], Ip, I1, recalc_table, QSZ::LENGTH_I_1CH);
  }
};
...
#include "qsz/qsz-1ch-triplets.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_T0_1CH, Invar(0, 0));
  }
} } break;
  case 2: { {
...
#include "qsz/qsz-2ch-triplets.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_T0_2CH, Invar(0, 0));
  }
} } break;
  case 3: { {
...
#include "qsz/qsz-3ch-triplets.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_T0_3CH, Invar(0, 0));
  }
} } break;
  default: my_assert_not_reached();
  };
...
#include "qsz/qsz-1ch-tripletp.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_Tpm_1CH, Invar(0, -2));
  }
} } break;
  case 2: { {
...
#include "qsz/qsz-2ch-tripletp.dat"
    ...
    recalc_general(diag, qsrmax, cold, cnew, I1, Ip, recalc_table, QSZ::LENGTH_Tpm_2CH, Invar(0, -2));
  }
} } break;
  default: my_assert_not_reached();
  };
...
void SymmetryQSZ::recalc_global(const DiagInfo &diag, const QSrmax &qsrmax, string name, MatrixElements &cnew) {
  if (name == "SZtot") {
    for(const auto &[I1, eig]: diag) {
