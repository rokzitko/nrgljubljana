...
void SymmetryISOSZLR::recalc_irreduc(const DiagInfo &diag) {
  ...
  LOOP_const(diag, isp) {
    ...
    I1 = Invar(iip + 1, sszp + 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isoupa.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
...
#include "isoszlr/isoszlr-2ch-spinup-isoupa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isoupb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isoupb.dat"
    ...
    I1 = Invar(iip + 1, sszp - 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isoupa.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isoupa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isoupb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isoupb.dat"
    ...
    I1 = Invar(iip - 1, sszp + 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isodowna.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isodowna.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isodownb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isodownb.dat"
    ...
    I1 = Invar(iip - 1, sszp - 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isodowna.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isodowna.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isodownb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isodownb.dat"
    ...
    I1 = Invar(iip + 1, sszp + 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isoupdiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isoupdiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isoupdiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isoupdiffb.dat"
    ...
    I1 = Invar(iip + 1, sszp - 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isoupdiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isoupdiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isoupdiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isoupdiffb.dat"
    ...
    I1 = Invar(iip - 1, sszp + 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isodowndiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isodowndiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spinup-isodowndiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spinup-isodowndiffb.dat"
    ...
    I1 = Invar(iip - 1, sszp - 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isodowndiffa.dat"
                << ", ch=" << 0 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "isoszlr/isoszlr-2ch-spindown-isodowndiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "isoszlr/isoszlr-2ch-spindown-isodowndiffb.dat"
                << ", ch=" << 1 << ", len=" << ISOSZLR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
...
void SymmetryISOSZLR::recalc_doublet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    ...
    Ip = Invar(ii1 - 1, ssz1 + 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-doubletmp.dat"
                << ", len=" << ISOSZLR::LENGTH_D_2CH << ", Iop=" << Invar(2, -1, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isoszlr/isoszlr-2ch-doubletmp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISOSZLR::LENGTH_D_2CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, ISOSZLR::LENGTH_D_2CH, Invar(2, -1, +1));
      }
    };
    ...
    Ip = Invar(ii1 - 1, ssz1 - 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-doubletmm.dat"
                << ", len=" << ISOSZLR::LENGTH_D_2CH << ", Iop=" << Invar(2, +1, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isoszlr/isoszlr-2ch-doubletmm.dat"
    ...
    Ip = Invar(ii1 + 1, ssz1 + 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-doubletpp.dat"
                << ", len=" << ISOSZLR::LENGTH_D_2CH << ", Iop=" << Invar(2, -1, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isoszlr/isoszlr-2ch-doubletpp.dat"
    ...
    Ip = Invar(ii1 + 1, ssz1 - 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-doubletpm.dat"
                << ", len=" << ISOSZLR::LENGTH_D_2CH << ", Iop=" << Invar(2, +1, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
...
void SymmetryISOSZLR::recalc_triplet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    ...
    Ip = Invar(ii1, ssz1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-triplets.dat"
                << ", len=" << ISOSZLR::LENGTH_T0_2CH << ", Iop=" << Invar(1, 0, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isoszlr/isoszlr-2ch-triplets.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISOSZLR::LENGTH_T0_2CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, ISOSZLR::LENGTH_T0_2CH, Invar(1, 0, +1));
      }
    };
    ...
    Ip = Invar(ii1, ssz1 + 2, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-tripletp.dat"
                << ", len=" << ISOSZLR::LENGTH_Tpm_2CH << ", Iop=" << Invar(1, -2, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "isoszlr/isoszlr-2ch-tripletp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == ISOSZLR::LENGTH_Tpm_2CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, ISOSZLR::LENGTH_Tpm_2CH, Invar(1, -2, +1));
      }
    };
    ...
    Ip = Invar(ii1, ssz1 - 2, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "isoszlr/isoszlr-2ch-tripletm.dat"
                << ", len=" << ISOSZLR::LENGTH_Tpm_2CH << ", Iop=" << Invar(1, +2, +1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
