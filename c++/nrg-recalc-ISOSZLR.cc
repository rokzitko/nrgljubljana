...
Opch SymmetryISOSZLR::recalc_irreduc(const Step &step, const DiagInfo &diag, const QSrmax &qsrmax) {
  Opch opch = newopch(P);
...
#include "isoszlr/isoszlr-2ch-doubletmp.dat"
        ...
        cnew[II] = this->recalc_general(diag, qsrmax, cold, I1, Ip, recalc_table, ARRAYLENGTH(recalc_table), Invar(2, -1, +1));
...
#include "isoszlr/isoszlr-2ch-triplets.dat"
        ...
        cnew[II] = this->recalc_general(diag, qsrmax, cold, I1, Ip, recalc_table, ARRAYLENGTH(recalc_table), Invar(1, 0, +1));
      }(); // immediately executed lambda
    }
  }
};
