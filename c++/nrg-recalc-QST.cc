...
void SymmetryQST::recalc_doublet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  ...
  LOOP(diag, is1) {
    ...
    Ip = Invar(q1 - 1, ss1 + 1, t1 - 1);
    {
      nrglog('f',
             "RECALC(fn="
                << "qst/qst-doubletp-1.dat"
                << ", len=" << QST::LENGTH_D_3CH_a << ", Iop=" << Invar(1, 2, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "qst/qst-doubletp-1.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QST::LENGTH_D_3CH_a);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, QST::LENGTH_D_3CH_a, Invar(1, 2, 1));
      }
    };
...
#include "qst/qst-doubletm-1.dat"
    ...
    Ip = Invar(q1 - 1, ss1 + 1, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "qst/qst-doubletp0.dat"
                << ", len=" << QST::LENGTH_D_3CH_b << ", Iop=" << Invar(1, 2, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "qst/qst-doubletp0.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QST::LENGTH_D_3CH_b);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, QST::LENGTH_D_3CH_b, Invar(1, 2, 1));
      }
    };
...
void SymmetryQST::recalc_irreduc(const DiagInfo &diag) {
  ...
  LOOP_const(diag, isp) {
    ...
    I1 = Invar(qp + 1, ssp + 1, tp + 1);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "qst/qst-spinup+1.dat"
                << ", ch=" << 0 << ", len=" << QST::LENGTH_I_3CH_0 << ")");
      if (diag.count(I1)) {
...
void SymmetryQST::recalc_triplet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    Invar I1   = INVAR(is1);
    Number q1  = I1.get("Q");
    Sspin ss1  = I1.get("SS");
    ...
    Ip = Invar(q1, ss1, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "qst/qst-triplets.dat"
                << ", len=" << QST::LENGTH_T0_3CH << ", Iop=" << Invar(0, 3, 0) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "qst/qst-triplets.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QST::LENGTH_T0_3CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, QST::LENGTH_T0_3CH, Invar(0, 3, 0));
      }
    };
    ...
    Ip = Invar(q1, ss1 + 2, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "qst/qst-tripletp.dat"
                << ", len=" << QST::LENGTH_Tpm_3CH << ", Iop=" << Invar(0, 3, 0) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "qst/qst-tripletp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QST::LENGTH_Tpm_3CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, QST::LENGTH_Tpm_3CH, Invar(0, 3, 0));
      }
    };
...
// Recalculate matrix elements of a triplet tenzor operator
void SymmetryQST::recalc_orb_triplet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  ...
  LOOP(diag, is1) {
    ...
    Ip = Invar(q1, ss1, t1);
    {
      nrglog('f',
             "RECALC(fn="
                << "qst/qst-orb-triplets.dat"
                << ", len=" << QST::LENGTH_OT0_3CH << ", Iop=" << Invar(0, 1, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "qst/qst-orb-triplets.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QST::LENGTH_OT0_3CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, QST::LENGTH_OT0_3CH, Invar(0, 1, 1));
      }
    };
    ...
    Ip = Invar(q1, ss1, t1 + 1);
    {
      nrglog('f',
             "RECALC(fn="
                << "qst/qst-orb-tripletp.dat"
                << ", len=" << QST::LENGTH_OTpm_3CH << ", Iop=" << Invar(0, 1, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "qst/qst-orb-tripletp.dat"
        };
        BOOST_STATIC_ASSERT(ARRAYLENGTH(recalc_table) == QST::LENGTH_OTpm_3CH);
        recalc_general(diag, cold, cnew, I1, Ip, recalc_table, QST::LENGTH_OTpm_3CH, Invar(0, 1, 1));
      }
    };
