...
void SymmetrySPSU2LR::recalc_doublet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    ...
    Ip = Invar(ss1 + 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2lr/spsu2lr-2ch-doubletp.dat"
                << ", len=" << SPSU2LR::LENGTH_D_2CH << ", Iop=" << Invar(-1, 1) << ")");
      if (diag.count(Ip)) {
...
#include "spsu2lr/spsu2lr-2ch-doubletp.dat"
    ...
    Ip = Invar(ss1 - 1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2lr/spsu2lr-2ch-doubletm.dat"
                << ", len=" << SPSU2LR::LENGTH_D_2CH << ", Iop=" << Invar(+1, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
...
void SymmetrySPSU2LR::recalc_irreduc(const DiagInfo &diag) {
  LOOP_const(diag, isp) {
    ...
    I1 = Invar(ssp + 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spinupa.dat"
                << ", ch=" << 0 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
...
#include "spsu2lr/spsu2lr-2ch-spinupa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spinupb.dat"
                << ", ch=" << 1 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-spinupb.dat"
    ...
    I1 = Invar(ssp - 1, pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spindowna.dat"
                << ", ch=" << 0 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-spindowna.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spindownb.dat"
                << ", ch=" << 1 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-spindownb.dat"
    ...
    I1 = Invar(ssp + 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spinupdiffa.dat"
                << ", ch=" << 0 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-spinupdiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spinupdiffb.dat"
                << ", ch=" << 1 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-spinupdiffb.dat"
    ...
    I1 = Invar(ssp - 1, -pp);
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spindowndiffa.dat"
                << ", ch=" << 0 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-spindowndiffa.dat"
    ...
    };
    {
      nrglog('f',
             "RECALC_F(fn="
                << "spsu2lr/spsu2lr-2ch-spindowndiffb.dat"
                << ", ch=" << 1 << ", len=" << SPSU2LR::LENGTH_I_2CH << ")");
      if (diag.count(I1)) {
        struct Recalc_f recalc_table[] = {
...
void SymmetrySPSU2LR::recalc_triplet(DiagInfo &diag, MatrixElements &cold, MatrixElements &cnew) {
  LOOP(diag, is1) {
    ...
    Ip = Invar(ss1, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2lr/spsu2lr-2ch-triplets.dat"
                << ", len=" << SPSU2LR::LENGTH_T0_2CH << ", Iop=" << Invar(0, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-triplets.dat"
    ...
    Ip = Invar(ss1 + 2, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2lr/spsu2lr-2ch-tripletp.dat"
                << ", len=" << SPSU2LR::LENGTH_Tpm_2CH << ", Iop=" << Invar(-2, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
#include "spsu2lr/spsu2lr-2ch-tripletp.dat"
    ...
    Ip = Invar(ss1 - 2, p1);
    {
      nrglog('f',
             "RECALC(fn="
                << "spsu2lr/spsu2lr-2ch-tripletm.dat"
                << ", len=" << SPSU2LR::LENGTH_Tpm_2CH << ", Iop=" << Invar(+2, 1) << ")");
      if (diag.count(Ip)) {
        struct Recalc recalc_table[] = {
