 ...
 public:
   ...
   // determined by 'SPIN'. For SPIN=0 both spin direction are equivalent. For QSZ, we need to differentiate the two.
    void calc(const Step &step, const DiagInfo<S> &diag,
      ...
      for (const auto &[II, op1_mat] : op1) {
        const auto &[Ij, Ii] = II;
        const auto op2_it = op2.find(II);
        if (op2_it == op2.end()) continue;
        if (!cf(Ij, Ii, spin)) continue;
        if (!Sym->project_subspace(Ii, P.project) || !Sym->project_subspace(Ij, P.project)) continue;
        const auto diagi = diag.find(Ii);
        const auto diagj = diag.find(Ij);
        if (diagi == diag.end() || diagj == diag.end()) continue;
        algo->calc(step, diagi->second, diagj->second, op1_mat, op2_it->second, ff(Ii, Ij), Ii, Ij, rho_here, stats); // stats.Zft needed
      }
      algo->end(step);
    }
};
