...
while (!(eof($F1) || eof($F2))) { 
	...
	my @numbers1 = split(/ +/,$line1);
	my @numbers2 = split(/ +/,$line2);
	my $len1 = @numbers1;
	my $len2 = @numbers2;
	if ($verbose >= 2) {
	    print "@numbers1 [$len1]\n";
	...
	if ($len1 != $len2) {
		...
		if (isnumeric($n1) && isnumeric($n2)) {
		    ...
		    # $error = 1; ### This is not (necessary) an error!
		    if (!isnumeric($n1) && isnumeric($n2)) {
...
if ($verbose >= 2) {
  print ("$cnt lines. $cntmatch identical. ");
  print ("$cntnumeric numerically equivalent.\n");
  print ("Error=$error\n");
}
