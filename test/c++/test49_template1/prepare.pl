...
# True hybridisation strength
my $gamma = `getparam Gamma`;
chomp($gamma);
print "gamma=$gamma\n";

# The value in theta.dat: 2gamma (integral of the hybridisation strength
# over the [-1:1] interval).
my $theta = 2*$gamma;
print "theta=$theta\n";
system "echo $theta >theta.dat";

my $replacement = sqrt($theta/pi);
